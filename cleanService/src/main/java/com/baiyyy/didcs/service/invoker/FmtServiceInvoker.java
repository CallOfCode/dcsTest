...
import com.baiyyy.didcs.dao.invoker.FmtInvokerMapper;
 ...
 */
@Service
public class FmtServiceInvoker extends AbstractStageServiceInvoker implements IStageServiceInvoker {
    private Logger logger = LoggerFactory.getLogger(FmtServiceInvoker.class);
    @Autowired
    LockService lockService;
    @Autowired
    DiscoveryClient discoveryClient;
    @Autowired
    FmtFlowService fmtFlowService;
    @SuppressWarnings("SpringJavaAutowiringInspection")
    @Autowired
    FmtInvokerMapper fmtInvokerMapper;
    ...
    public List<Map<String ,Object>> getHeadIds(int threads,Map<String,Object> param){
        ...
        try{
            String notNullSql = fmtFlowService.getNotNullSqlByBatchId(MapUtil.getCasedString(param.get("batchId")));
            //1.获取最大最小值及数据总量
            Map codeMap = fmtFlowService.getCodesByBatchId(MapUtil.getCasedString(param.get("batchId")));
            Map totalMap = fmtInvokerMapper.selectMaxMinTotal(MapUtil.getCasedString(codeMap.get("source_code")), MapUtil.getCasedString(codeMap.get("task_code")),MapUtil.getCasedString(codeMap.get("batch_code")),MapUtil.getCasedString(param.get("minId"),"0"),MapUtil.getCasedString(param.get("maxId")), notNullSql);
            ...
            if(null==total || total==0){
                ...
                for(int i=0;i<threads;i++){
                    ...
                    if(i==threads-1){
                        //最后一段则不使用Limit参数
                        tmpIdMap = fmtInvokerMapper.selectMaxMinLimit(MapUtil.getCasedString(codeMap.get("source_code")), MapUtil.getCasedString(codeMap.get("task_code")),MapUtil.getCasedString(codeMap.get("batch_code")),String.valueOf(nextMinId), String.valueOf(maxId), null, notNullSql);
                    }else{
                        tmpIdMap = fmtInvokerMapper.selectMaxMinLimit(MapUtil.getCasedString(codeMap.get("source_code")), MapUtil.getCasedString(codeMap.get("task_code")),MapUtil.getCasedString(codeMap.get("batch_code")),String.valueOf(nextMinId), String.valueOf(maxId), String.valueOf(splitSize), notNullSql);
