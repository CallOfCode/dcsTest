layui.config({
	...
	//加载页面数据
	var schemasData = '';
	$.get("/schemas/list", function(data){
        schemasData = data;
        if(window.sessionStorage.getItem("addNews")){
            var addNews = window.sessionStorage.getItem("addNews");
            schemasData = JSON.parse(addNews).concat(schemasData);
        }
        //执行加载数据的方法
        newsList();
	})
    ...
    function newsList(that){
        ...
        function renderDate(data,curr){
            var dataHtml = '';
            if(!data){
                currData = schemasData.concat().splice(curr*nums-nums, nums);
            }else{
        ...
        laypage({
            cont : "page",
            pages : Math.ceil(schemasData.length/nums),
            jump : function(obj){
                $(".news_content").html(renderDate(schemasData,obj.curr));
            ...
            setTimeout(function(){
            	$.ajax({
					...
					success : function(data){
						if(window.sessionStorage.getItem("addNews")){
							var addNews = window.sessionStorage.getItem("addNews");
							schemasData = JSON.parse(addNews).concat(data);
						}else{
							schemasData = data;
						}
						for(var i=0;i<schemasData.length;i++){
							var newsStr = schemasData[i];
		            	...
		            	schemasData = newArray;
		            	newsList(schemasData);
					}
				})
		...
		if($checkbox.is(":checked")){
            ...
            setTimeout(function(){
            	for(var j=0;j<$checked.length;j++){
            		for(var i=0;i<schemasData.length;i++){
						if(schemasData[i].newsId == $checked.eq(j).parents("tr").find(".news_del").attr("data-id")){
							//修改列表中的文字
	...
	$("body").on("click",".news_del",function(){  //删除
		...
		layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
			...
			for(var i=0;i<schemasData.length;i++){
				if(schemasData[i].newsId == _this.attr("data-id")){
					schemasData.splice(i,1);
					newsList(schemasData);
				}
			}
